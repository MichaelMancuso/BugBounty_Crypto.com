...
|----------|-------------|
| Screenshot 1 | Terminal: HTTP 201 for `<details ontoggle>` XSS payload on `/logs` |
| Screenshot 2 | Terminal: HTTP 201 for `<details ontoggle>` XSS payload on `/traces` |
| Screenshot 3 | Terminal: HTTP 201 for `<details ontoggle>` XSS payload on `/metrics` |
| Screenshot 4 | Terminal: HTTP 403 for raw `<img onerror=>` (WAF block confirmation) |
| Screenshot 5 | Terminal: side-by-side 403 vs 201 (WAF bypass proof, PoC Step 2) |
| Screenshot 6 | Terminal: 20× rate limit probe — all 201, no 429 |
